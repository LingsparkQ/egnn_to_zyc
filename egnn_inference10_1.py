...
names = locals()
num = '10'
dir = "/work/01/gq54/p23002/guyuhan/egnn/826_Alchemy" + num + "_for_dimenet.npz"
name = '906_egnn_inference_al' + num + '_1.npz'
data=np.load(dir)
data1 = {}
for _ in data.files:
    data1[_] = data[_][0:100]
# print(data1.files)
n = len(data1['mu'])
# n = 100
...
prop = ['mu', 'alpha', 'homo', 'lumo', 'gap', 'r2',
        'zpve', 'U0', 'U','H', 'G', 'Cv' ]
split = [data1['N'][0]]
for i in tqdm(range(len(data1['N'])-2)):
    split.append(split[-1] + data1['N'][i + 1])
sum = split[-1] + data1['N'][i + 2]
# print(len(split))
num_atoms_split = data1['N']
max = num_atoms_split.max()
charges_split = np.split(data['Z'][0:sum],split)
charge_scale = data1['Z'].max()
positions_split = np.split(data['R'][0:sum],split)
index_split = data1['id']
for _ in prop:
        names[_ + '_split'] = data1[_]
...
dataset_alchemy = []
small = list(range(0,len(data1['homo'])))
for i in small:
    dict_i = {}
    for _ in dict_keys1:
...
for ip in infer_props:
    ...
    mae = abs((pred_all)[0:len(small)] - label[0:len(small)]).mean()
    coef = np.corrcoef(pred_all[0:len(small)],label[small])
